...
struct inode *rcfs_get_inode(struct super_block *sb,
...
{
	struct inode * inode = rcfs_get_inode(dir->i_sb, dir, mode, dev);
	int error = -ENOSPC;
			 ...
			 struct dentry *dentry, const char *symname)
{
	...
	inode = rcfs_get_inode(dir->i_sb, dir, S_IFLNK|S_IRWXUGO, 0);
	if (inode) {
			 ...
			 struct inode *dir, struct file *file, umode_t mode)
{
	...
	inode = rcfs_get_inode(dir->i_sb, dir, mode, 0);
	if (!inode)
		return -ENOSPC;
...
const struct inode_operations rcfs_dir_inode_operations = {
	.create		= ramfs_create,
...
const struct fs_parameter_spec rcfs_fs_parameters[] = {
...
static int ramfs_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
	...
	opt = fs_parse(fc, rcfs_fs_parameters, param, &result);
	if (opt == -ENOPARAM) {
...
static int ramfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	inode = rcfs_get_inode(sb, NULL, S_IFDIR | fsi->mount_opts.mode, 0);
...
int rcfs_init_fs_context(struct fs_context *fc)
{
...
static struct file_system_type ramfs_fs_type = {
	.name		= "rcfs",
	.init_fs_context = rcfs_init_fs_context,
	.parameters	= rcfs_fs_parameters,
